...
| | |
|---|---|
| C41 | **A block that does not fill its band is extended with its own last cell.** `[a, [3 4]]`, with `a` 2x2, gives `1 2 3 4 / 3 4 4 4`: the second row under the 1x2 block is its last cell twice. `[[1 2 3], a]` fills a whole row with `3 3 3`. The code says so in as many words -- *"extend the previous (up and left) evaluated cell result"* -- so it is deliberate, but it is the third rule for the same question and the only one nobody can state: a literal's short row pads with zeros (README.md section 2), a single value stretches to its band (which is what makes `[a, 1]` mean what it looks like, and what C38 just extended to `+`, `-` and `/`), and a short block repeats a corner. The last is not a mathematical operation on matrices and produces numbers the user never wrote. **Proposed:** keep the stretch for a 1x1, and make any other block that does not fill its band a diagnostic naming the two sizes. Zero-padding is the alternative, consistent with the short-row rule, but it invents values too. Recorded as it is in `matrices.ink` meanwhile, with the entry saying so. Found by writing `[a, [3 4]]` while testing something else. |
| C40 `[fixed]` | **A matrix could be built and never read.** `a(1,2)` was `a takes no arguments` and `a_1` was `a is not a sequence`: the language had no way at all to get a value back out of a matrix, which is half of what the second of the three ideas is for. `Matrix::operator()` was there, with a good out-of-range message, and nothing in the language reached it. `m[i,j]` now does, one-based as the rows and columns are written, and composing with everything a name can carry -- `f(3)[1,2]` and `s_3[1,2]` both work. The brackets were chosen over parentheses to match array indexing elsewhere, and they bind to a name alone: a space between two blocks already means something, so `[a [3 4]]` was a row of two blocks and is now read as an index. That form was legal, unused in the corpus and in `README.md`, and is written `[a, [3 4]]` instead; the change turns it into a diagnostic rather than a wrong answer. `Matrix::Offset` takes a signed index so that `m[0-1,1]` names the row it asked for instead of one that wrapped. |
